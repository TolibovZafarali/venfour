...
class ConfirmedValuationInputTests(unittest.TestCase):
    ...
    def test_manual_input_requires_explicit_condition_and_options_responses(self) -> None:
        for field in ("vehicle_condition", "vehicle_options_packages"):
            with self.subTest(field=field), self.assertRaises(ValuationInputError):
                ConfirmedValuationInput.from_snapshot(snapshot(**{field: None}))
