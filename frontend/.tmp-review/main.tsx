...
import {createBrowserRouter,RouterProvider,useParams,useLocation,useNavigate} from 'react-router';
...
function Harness(){const{stage='result'}=useParams();const location=useLocation();const navigate=useNavigate();const query=useTotalLossClaimQuery({accessToken:session.access_token,caseId:CASE_ID,userId:USER_ID});useEffect(()=>localStorage.setItem(`${storageKey}-path`,`${location.pathname}${location.search}`),[location.pathname,location.search]);useEffect(()=>{if(query.data?.state==='secured'&&query.data.journey?.nextState==='awaiting_insurer_response'&&location.pathname.endsWith('/review/request'))navigate(`${BASE}/review/waiting`,{replace:true});},[location.pathname,navigate,query.data]);if(query.isError)return <p role="alert">{query.error.message}</p>;if(!query.data||query.data.state!=='secured'||!query.data.report)return <p>Loading synthetic report…</p>;return <CompletedAnalysis accessToken={session.access_token} caseId={CASE_ID} claim={query.data} intakeMode={mode} onRefresh={query.refetch} report={query.data.report} userId={USER_ID} view={`review_${stage.replaceAll('-','_')}` as any}/>;}
