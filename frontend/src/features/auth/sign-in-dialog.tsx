...
function SignInExperience({
  ...
  const content = <>
    <div className={inline ? "partner-sign-in-heading" : "pr-10"}>
      {inline && <p className="partner-sign-in-eyebrow">Venfour for businesses</p>}
      <Title className="text-xl font-semibold tracking-[-0.025em] text-ink">
        {emailSent ? "Check your email" : inline ? "Sign in to your business account" : "Sign in to Venfour"}
      </Title>
      <Description className="mt-2 text-sm leading-6 text-copy">
        {emailSent
          ? "Enter the code from your email to finish signing in."
          : inline ? "Use the email address that received your invitation." : intentDescriptions[intent]}
      </Description>
    </div>
    ...
    {auth.status === "unavailable" ? (
      ...
      <div className="mt-6">
        {inline && <>{emailForm}<div className="partner-sign-in-divider"><span>or continue with</span></div></>}
        <button
          ref={googleButtonRef}
          type="button"
        ...
        {!inline && <div className="my-5 flex items-center gap-3" aria-hidden>
