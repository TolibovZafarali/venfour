...
import { canCloseCase, currentAcceptedOffer, resolutionAmount, resolutionOutcome } from "../resolution";
...
export function CaseResolutionBanner({ resolution }: { readonly resolution: TotalLossCaseResolution }) {
  return <section className="case-resolution-banner" aria-label="Recorded case outcome">
    ...
    {resolution.amountMinorUnits !== null && resolution.currency ? <p className="case-resolution-amount">
      ...
      <span>{resolution.amountSource === "VERIFIED_INSURER_OFFER" ? "Exact verified insurer offer · acceptance confirmed by you" : "Final amount reported by you"}</span>
    </p> : null}
...
function ClosureConfirmation({ accepted = false, ...props }: ResolutionIdentity & { readonly accepted?: boolean }) {
  ...
  const verified = currentAcceptedOffer(claim);
  ...
  const confirm = async () => {
    if (locked.current || stale || confirmed || !canCloseCase(claim) || !claim.workflow || (accepted && !verified)) return;
    ...
    const input: TotalLossCaseResolutionInput = attemptRef.current ?? {
      ...
      decisionId: accepted ? verified!.decision.decisionId : null,
      offerId: accepted ? verified!.offer.offerId : null,
  ...
  return <form className="case-closure-confirmation" aria-label={accepted ? "Confirm accepted offer" : "Choose case outcome"} onSubmit={(event) => { event.preventDefault(); void confirm(); }}>
    {accepted && verified ? <>
      ...
      <p>You accepted <strong>{resolutionAmount(verified.offer.amountMinorUnits, verified.offer.currency)} {verified.offer.currency}</strong> with your insurer. This confirmation will resolve your Venfour case and keep its records available to review.</p>
    </> : <>
      <fieldset disabled={pending || Boolean(attempt)}>
        ...
        {outcome === "RESOLVED_WITH_INSURER" ? <label className="case-closure-amount">Final amount ({currency}, optional)
          ...
          <span id="case-closure-amount-note">This amount is your report of the outcome. It does not replace verified insurer evidence or change the valuation.</span>
...
export function AcceptedOfferFinalization(props: ResolutionIdentity) {
  ...
  const verified = currentAcceptedOffer(props.claim);
  ...
  return <section className="case-finalization" aria-label="Complete acceptance with your insurer">
    ...
    <dl className="case-finalization-offer"><dt>The insurer offer you chose</dt><dd>{resolutionAmount(verified.offer.amountMinorUnits, verified.offer.currency)} {verified.offer.currency}</dd></dl>
    <p>Your case remains open until you explicitly confirm. Venfour does not communicate acceptance to your insurer.</p>
