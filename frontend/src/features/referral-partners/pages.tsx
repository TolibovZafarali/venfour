...
export function PartnerDetailPage() {
  ...
  return <><div className="partner-page-header"><div><h1>{detail.partner.business_name}</h1><p>{partnerStatusLabel(detail.partner.status)}</p></div><Link to="/partners" className="partner-text-button">Your workspace</Link></div>{detail.partner.status !== "onboarding" && <PartnerBusinessSummary detail={detail} />}{detail.partner.status === "onboarding" && <ProfileForm key={detail.partner.id} partner={detail.partner} />}{!current && detail.partner.status === "onboarding" && <PreparePartnerAgreement partner={detail.partner} />}{current && detail.partner.status === "onboarding" && <><section className="partner-card"><AgreementText agreement={current} /></section><SignatureForm key={current.id} audience="partner" agreement={current} partnerId={partnerId} /></>}{detail.partner.status === "awaiting_approval" && <p className="partner-note">Your signature is saved. Venfour will review and countersign your agreement. You can read your signed agreement below.</p>}{detail.partner.status === "active" && <p className="partner-note">Your partner onboarding is complete. Share your referral link and follow submitted reviews below. Your completed agreement remains available in your agreement history.</p>}{detail.partner.status === "active" && <ReferralTracking key={partnerId} partnerId={partnerId} audience="partner" />}<PartnerHistory detail={detail} audience="partner" /></>;
}
