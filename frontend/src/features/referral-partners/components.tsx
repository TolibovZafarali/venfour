...
import { IntakeProgress } from "@/features/total-loss/intake-fields";
...
export function PartnerOnboardingProgress({ current }: { current: 1 | 2 | 3 }) {
  return <div className="partner-onboarding-progress"><IntakeProgress current={current}
    steps={["Business details", "Agreement", "Venfour approval"]}
    ariaLabel="Partnership setup progress" stepsAriaLabel="Partnership setup steps" /></div>;
}
...
export function PartnerBusinessSummary({ detail }: { detail: PartnerDetail }) {
  ...
  return <section className="partner-card" aria-label="Agreed business details"><h2>Your partnership details</h2><dl className="partner-summary"><div><dt>Legal business name</dt><dd>{String(snapshot.legal_business_name ?? snapshot.business_name ?? "")}</dd></div><div><dt>Agreed commission</dt><dd>{isOutcomeCommissionPolicy(snapshot.commission_policy) ? "$50 / $75 per verified successful case" : typeof snapshot.commission_amount_minor_units === "number" ? formatPartnerMoney(snapshot.commission_amount_minor_units) : "See agreement"}</dd></div><div><dt>Verified contact email</dt><dd>{String(snapshot.contact_email ?? "")}</dd></div><div><dt>Contact</dt><dd>{String(snapshot.contact_name ?? "")}<br />{String(snapshot.contact_title ?? "")}</dd></div><div><dt>Business address</dt><dd>{[snapshot.address_line1, snapshot.address_line2, snapshot.city, snapshot.state, snapshot.postal_code, snapshot.country].filter((value) => typeof value === "string" && value).join(", ")}</dd></div></dl></section>;
}
...
export function AgreementBusinessDetails({ agreement }: { agreement: PartnerAgreement }) {
  return <dl className="partner-summary"><div><dt>Legal business name</dt><dd>{String(agreement.snapshot.legal_business_name ?? agreement.snapshot.business_name ?? "")}</dd></div><div><dt>Contact</dt><dd>{String(agreement.snapshot.contact_name ?? "")}<br />{String(agreement.snapshot.contact_title ?? "")}<br />{String(agreement.snapshot.contact_email ?? "")}</dd></div><div><dt>Business address</dt><dd>{[agreement.snapshot.address_line1, agreement.snapshot.address_line2, agreement.snapshot.city, agreement.snapshot.state, agreement.snapshot.postal_code, agreement.snapshot.country].filter((value) => typeof value === "string" && value).join(", ")}</dd></div>{!isOutcomeCommissionPolicy(agreement.snapshot.commission_policy) && typeof agreement.snapshot.commission_amount_minor_units === "number" && <div><dt>Commission per qualifying purchase</dt><dd>{formatPartnerMoney(agreement.snapshot.commission_amount_minor_units)}</dd></div>}</dl>;
}

export function AgreementText({ agreement, showBusinessDetails = true }: { agreement: PartnerAgreement; showBusinessDetails?: boolean }) {
  return <article className="partner-agreement" aria-label="Agreement for review">
    <h2>{agreement.snapshot.title}</h2>
    {agreement.snapshot.synthetic_preview === true && <p className="partner-note">Draft proposal · synthetic signing demonstration only. This does not execute an agreement or enable paid referrals.</p>}
    {isOutcomeCommissionPolicy(agreement.snapshot.commission_policy) && <CommissionSummary />}
    {showBusinessDetails && <AgreementBusinessDetails agreement={agreement} />}
...
export function SignatureForm({ audience, agreement, onSuccess, presentation = "card" }: { audience: PartnerAudience; agreement: PartnerAgreement; partnerId: string; onSuccess?: () => void; presentation?: "card" | "plain" }) {
  ...
  const submit = async (event: FormEvent) => {
    ...
    if (agreement.snapshot.release_hold === true || stale || !titleMatches || !Object.values(consents).every(Boolean)) return;
  ...
  return <form className={presentation === "plain" ? "partner-form partner-signature" : "partner-card partner-form"} onSubmit={(event) => void submit(event)}>
    <h2>{audience === "staff" ? "Countersign for Venfour" : "Sign your agreement"}</h2>
    <p>Review the complete agreement above. Your signature will be recorded with this version. Venfour must separately countersign and activate the partnership.</p>
    {agreement.snapshot.release_hold === true && <p role="status">This draft is held for owner review and cannot be signed.</p>}
    ...
    {([
      ["electronic_consent", "I have read and accept this agreement and intend to sign it electronically."],
    ...
    <Button disabled={agreement.snapshot.release_hold === true || mutation.pending || stale || !titleMatches || !Object.values(consents).every(Boolean)} type="submit">{mutation.pending ? "Saving signature…" : audience === "staff" ? "Countersign and activate" : "Agree and sign"}</Button>
  </form>;
}

export function AgreementDownload({ audience, agreement }: { audience: PartnerAudience; agreement: PartnerAgreement }) {
