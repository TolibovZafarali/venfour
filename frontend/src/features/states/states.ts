// Names and codes mirror venfour/data/nationwide_product_v1.json; checked by states.test.ts.
export const states = [
...
export function findState(slug: string | undefined): State | undefined {
...
export function stateFromPath(pathname: string) {
  const match = /^\/states\/([a-z-]+)\/?$/.exec(pathname);
  return match ? findState(match[1]) : undefined;
}
...
const footerCodes: readonly string[] = ["CA", "TX", "FL", "NY", "PA", "IL", "OH", "GA", "NC", "MO"];
export const footerStates = states.filter(state => footerCodes.includes(state.code));
