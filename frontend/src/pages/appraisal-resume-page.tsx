...
function ResolvedAppraisal({ service, userId, caseId }: { service: AppraisalCaseService; userId: string; caseId: string }) {
  ...
  if (query.isPending || query.isFetching) return <AppEntryLoading />;
...
export function AppraisalResumePage() {
  return <AppEntryLoadingScope><AppraisalResumeContent /></AppEntryLoadingScope>;
}

function AppraisalResumeContent() {
