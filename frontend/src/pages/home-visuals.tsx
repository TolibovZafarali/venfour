...
import {
  homepageExampleAppraisal,
  homepageExampleDifference,
} from "@/pages/home-example";
...
export function ExampleAppraisalVisual() {
  return (
    <figure
      ...
      <div className="p-5 sm:p-6">
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="border-l-2 border-slate-300 pl-4">
            ...
            <p className="mt-1 text-[2rem] leading-none font-semibold tracking-[-0.04em] text-ink tabular-nums sm:text-[2.35rem]">
              {currency.format(homepageExampleAppraisal.insuranceValue)}
            </p>
          </div>
          <div className="border-l-2 border-market pl-4">
            <p className="text-xs font-medium text-copy">Similar vehicles</p>
            <p className="mt-1 text-[1.55rem] leading-none font-semibold tracking-[-0.04em] whitespace-nowrap text-ink tabular-nums sm:text-[1.75rem]">
              {currency.format(homepageExampleAppraisal.marketMinimum)}–
              {currency.format(homepageExampleAppraisal.marketMaximum)}
            </p>
          </div>
        </div>

        <div className="mt-7" aria-label="Insurance value compared with the example market range">
          <div className="flex items-center justify-between text-[0.6875rem] font-medium text-copy tabular-nums">
            <span>$18,500</span>
            <span>$22,000</span>
          </div>
          <div className="relative mt-5 h-3 rounded-full bg-slate-200">
            <span
              className="absolute top-0 bottom-0 left-[65.7%] w-[22.9%] rounded-full bg-market/75"
              aria-hidden
            />
            <span
              className="absolute -top-3 left-[15.6%] h-9 w-0.5 bg-ink motion-safe:animate-[range-marker_500ms_ease-out_both]"
              aria-hidden
            />
            <span
              className="absolute -top-8 left-[15.6%] -translate-x-1/2 whitespace-nowrap rounded bg-ink px-2 py-1 text-[0.625rem] font-semibold text-white"
              aria-hidden
            >
              Report
            </span>
            <span
              className="absolute top-5 left-[77.1%] -translate-x-1/2 whitespace-nowrap text-[0.625rem] font-semibold text-market-strong"
              aria-hidden
            >
          ...
          <p className="sr-only">
            The insurance report value is {currency.format(homepageExampleAppraisal.insuranceValue)}.
            The example market range is {currency.format(homepageExampleAppraisal.marketMinimum)} to {currency.format(homepageExampleAppraisal.marketMaximum)}.
          </p>
        </div>

        <div className="mt-9 grid gap-3 rounded-xl border border-market/25 bg-market-soft p-4 sm:grid-cols-[1fr_auto] sm:items-center sm:gap-5">
          <div>
            <p className="text-xs font-semibold text-market-strong">
              Possible value difference
            </p>
            <p className="mt-1 text-2xl font-semibold tracking-[-0.035em] text-ink tabular-nums">
              {currency.format(homepageExampleDifference.minimum)}–
              {currency.format(homepageExampleDifference.maximum)}
            </p>
          </div>
          <div className="border-t border-market/20 pt-3 sm:border-t-0 sm:border-l sm:pt-0 sm:pl-5">
            <p className="text-[0.6875rem] font-medium text-copy">
              Vehicles reviewed
            </p>
            <p className="mt-0.5 text-xl font-semibold text-ink tabular-nums">
              {homepageExampleAppraisal.reviewedCount}
            </p>
          </div>
        </div>
        ...
        <div className="mt-5">
          <div className="mb-2 flex items-center justify-between gap-4">
            ...
            <p className="text-[0.6875rem] text-copy">3 of 8 shown</p>
          </div>
          <ul className="divide-y divide-slate-200 border-y border-slate-200">
            {homepageExampleAppraisal.vehicles.map((vehicle) => (
              ...
              >
                ...
                <p className="text-xs font-semibold text-ink tabular-nums sm:text-sm">
                  {vehicle.price}
                </p>
                <p className="text-[0.6875rem] text-copy">
                  {vehicle.mileage}
                </p>
                <p className="text-[0.6875rem] text-copy">
                  {vehicle.distance}
...
export function TotalLossServiceVisual() {
  return (
    <figure className="grid min-h-72 gap-3 rounded-xl border border-slate-200 bg-slate-100 p-4 sm:grid-cols-[minmax(0,0.9fr)_2.75rem_minmax(0,1.1fr)] sm:items-center sm:gap-4 sm:p-6">
      <p className="text-[0.625rem] font-semibold tracking-[0.1em] text-copy uppercase sm:col-span-3">
        Synthetic example
      </p>
      <div className="rounded-lg border border-slate-300 bg-white p-3 shadow-[0_12px_30px_-24px_rgba(11,31,51,0.45)] sm:p-4">
      ...
      <div className="rounded-lg border border-brand/25 bg-white p-3 shadow-[0_12px_30px_-24px_rgba(11,31,51,0.45)] sm:p-4">
        <div className="flex items-center gap-2 border-b border-slate-200 pb-3">
...
export function RepairedVehicleServiceVisual() {
  ...
  return (
    <figure className="rounded-xl border border-slate-300 bg-ink p-5 text-white sm:p-7">
...
export function AnnotatedInsuranceReportVisual() {
  ...
  return (
    <figure className="grid gap-5 rounded-2xl border border-slate-300 bg-slate-100 p-5 sm:p-7 lg:grid-cols-[minmax(0,1.15fr)_minmax(15rem,0.85fr)] lg:items-center">
      <div className="rounded-xl border border-slate-300 bg-white p-4 shadow-[0_20px_45px_-34px_rgba(11,31,51,0.55)] sm:p-6">
  ...
  return (
    <figure className="rounded-2xl border border-slate-300 bg-white p-5 shadow-[0_24px_55px_-42px_rgba(11,31,51,0.5)] sm:p-7 lg:p-8">
      <div className="grid items-center gap-5 sm:grid-cols-[1fr_auto_1fr_auto_1fr]">
        ...
        <ArrowDown className="mx-auto size-5 text-slate-400 sm:hidden" aria-hidden />
        <ArrowRight className="mx-auto hidden size-5 text-slate-400 sm:block" aria-hidden />
        <div className="rounded-xl border border-amber/35 bg-amber-soft p-4 text-center">
      ...
      <div className="mt-7 grid gap-5 border-t border-slate-200 pt-6 sm:grid-cols-[1fr_auto_1fr] sm:items-end">
        <div>
    ...
    <figure className="grid gap-5 rounded-2xl border border-slate-300 bg-slate-100 p-5 sm:p-7 lg:grid-cols-[minmax(0,1.1fr)_minmax(14rem,0.7fr)] lg:items-center lg:p-8">
      <div className="overflow-hidden rounded-xl border border-slate-300 bg-white shadow-[0_24px_55px_-40px_rgba(11,31,51,0.55)]">
        ...
        <div className="grid gap-4 p-4 sm:grid-cols-2 sm:p-5">
          ...
          <div className="rounded-lg border border-market/25 bg-market-soft p-3">
            <p className="text-[0.625rem] font-medium text-copy">
              Example market range
            </p>
            <p className="mt-1 text-lg font-semibold text-ink tabular-nums">
