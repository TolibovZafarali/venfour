...
describe("public website and optional advertising boundary", () => {
  const env = (enabled = false): Env => ({
    DEPLOYMENT_ENVIRONMENT: "public-site", GOOGLE_ADS_MEASUREMENT: String(enabled),
    ASSETS: { fetch: vi.fn(async () => new Response("public website", { headers: { "Content-Type": "text/html" } })) } as unknown as Fetcher,
  });
  it("publishes existing public pages in the sitemap", async () => {
    ...
    expect(sitemap).not.toContain("app.venfour.com");
  ...
  it("adds exact Google origins only when explicitly configured without permitting unsafe scripts", async () => {
    const response = await handleRequest(new Request("https://venfour.com/"), env(true));
