...
import {session,cases,detail,dvCase,dvQueue} from './fixtures';
import './styles.css';
const noop=async()=>{};
...
const dv={caseService:{isStaff:async()=>mode!=='denied',listSubmittedCases:()=>result(mode==='empty'?[]:dvQueue),getSubmittedCase:(id:string)=>result(id===dvCase.caseId?dvCase:null)},documentService:{listDocuments:async()=>[],downloadDocument:async()=>{throw new Error('No documents in this synthetic preview')}}};
function Preview(){return <><div className="preview-bar"><strong>Synthetic staff preview · fictional cases</strong><Link to="/admin/cases">All cases</Link><Link to="/admin/diminished-value">Diminished value</Link><label>Preview state <select aria-label="Preview state" value={mode} onChange={e=>{location.href=location.pathname+'?state='+e.target.value}}>{['populated','empty','loading','error','denied'].map(s=><option key={s}>{s}</option>)}</select></label></div><Outlet/></>}
const router=createBrowserRouter([{element:<Preview/>,children:[{element:<AppShell/>,children:[{path:'/',element:<Navigate to="/admin/cases" replace/>},{path:'/admin/cases',element:<AdminCaseOperationsAccessGate/>,children:[{index:true,element:<AdminCaseOperationsPage/>},{path:':caseId',element:<AdminTotalLossCasePage/>}]},{path:'/admin/diminished-value',element:<AdminDiminishedValueAccessGate/>,children:[{index:true,element:<AdminDiminishedValueQueuePage/>},{path:':caseId',element:<AdminDiminishedValueCasePage/>}]}]}]}]);
