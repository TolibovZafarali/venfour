      ...
      if (path === '/follow-up' && method === 'POST') return record(path, body, () => {
        ...
        if (!followUp.draft) {
          ...
          claim.followUp = { ...followUp, state: 'draft', draft: {
            ...
            body: `${claim.sendingDetails?.adjusterName ? `Hello ${claim.sendingDetails.adjusterName},` : 'Hello,'}\n\nThank you for reviewing my request for claim ${claim.sendingDetails?.claimReference ?? 'CLM-42'}.${amount ? ` I have recorded ${amount} as the vehicle valuation amount. Could you please confirm that amount?` : ''}\n\nCould you please explain how the comparable listings in the report were considered, including any relevant differences from my ${claim.report!.subjectVehicle.description}? I understand these are asking prices.\n\nI've attached the report again for convenience. Could you please reconsider the valuation in light of this evidence and reply with any updated valuation? If it remains unchanged, a brief explanation would help me understand.\n\nThank you for your time and help,\n${claim.sendingDetails?.customerName ?? 'Vehicle owner'}`,
            updatedAt: timestamp(),
          } };
