...
for(const [width,height] of [[1440,900],[390,844]]) {
 ...
 for(const [name,path] of [['home','/'],['contact','/contact'],['terms','/terms']]) {
  ...
  await page.screenshot({path:`output/visual-system/after-public-${name}-${width}.png`,fullPage:true,animations:'disabled'});
...
await fs.writeFile('output/visual-system/after-public.json',JSON.stringify(results,null,2));console.log(JSON.stringify(results,null,2));await browser.close();
