-- Expose separately labeled supporting examples and bounded search context.
-- Historical report projections retain their previous exact field set.
...
begin
  ...
  return jsonb_build_object(
    ...
    'marketEvidence', jsonb_build_object(
      ...
      when report_row.report #> '{independentMarketEvidence,marketSearchContext}' is not null
      then jsonb_build_object('marketSearchContext', report_row.report #> '{independentMarketEvidence,marketSearchContext}')
      else '{}'::jsonb end
      || case
      when report_row.report #> '{independentMarketEvidence,higherPricedComparableListings}' is not null
      then jsonb_build_object('higherPricedComparableListings', report_row.report #> '{independentMarketEvidence,higherPricedComparableListings}')
      else '{}'::jsonb end
  );
end;
$$;
